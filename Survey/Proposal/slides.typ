...
特别值得一提的是 WalkSATlm @caiImprovingWalkSATEffective2015 的改进，它通过引入基于 lmake 的平局打破机制@Cai2013ComprehensiveS @Cai2014ScoringFB @Prestwich2005RandomWW，在随机5-SAT和7-SAT实例上都实现了性能突破，超越了当时最先进的求解器。
...
在面对基数约束这一挑战时，研究界提出了两类创新性解决方案。首先，基于SAT & LCG/SMT的混合求解方法@abioConflictDirectedLazy2012 引入了冲突驱动的惰性分解策略，巧妙地平衡了约束分解与直接传播两种方法的优势。该策略在处理冲突时，能够根据中间文字的有效性自动切换策略：当中间文字对解决冲突无益时，采用传播方法；而当其有助于解决冲突时，则启用分解方法。这种自适应机制确保了求解效率不会显著低于两种基本方法中的更优者，甚至在某些情况下超越了它们。然而，由于该方法依赖于DPLL(T)框架@Nieuwenhuis2006SolvingSA，其在处理大规模基数约束时仍面临挑战，尤其是在扩展性和效率方面表现不足。

其次，基于局部搜索的LS-ECNF方法 @Lei2020ExtendedCN 提供了一种全新的视角。该方法摒弃了传统的编码转换思路，通过扩展合取范式（ECNF）直接表示基数约束，极大地简化了问题表示。ECNF模型通过引入约束数组，自然地描述了基数约束，无需额外的变量或子句，从而降低了问题的复杂性。此外，LS-ECNF 还设计了适应性评分函数和基于广义单元传播的初始化策略，并在实验中 LS-ECNF 通过与各类编码方式，PB 求解以及基于 DPLL 的精确求解器进行对比，LS-ECNF 在多个维度上超越了现有的 SAT 求解器，证明了专有求解器的必要性。
...
随机局部搜索@caiImprovingWalkSATEffective2015 是求解 SAT 的经典算法。该算法是不完备的，因为算法无法确定给定公式是否满足。 局部搜索的一般方法包括从随机赋值开始，然后翻转变量，直到找到令人满意的赋值。

变量翻转的策略可以分为以下几种：
- 随机翻转：随机选择一个变量进行翻转。
- 启发式搜索：根据启发式信息选择变量，以期增加满足的子句数量。
\




WalkSATlm @caiImprovingWalkSATEffective2015 算法是一个典型的局部搜索算法，其框架如 @fig:walksat 所示：

+ 初始化：首先对变量进行随机赋值。
+ 迭代过程：随机选择一个未满足的子句。
- 如果存在翻转后能增加满足子句数量的变量，则翻转该变量；
- 否则，以概率 $1 - p$ 选择一个最小代价的变量，或以概率 $p$ 随机选择一个变量进行翻转。

参数 $p$ 控制着算法的贪心程度和随机性。当 $p = 0$ 时，算法完全贪心；当 $p = 1$ 时，算法完全随机，有助于逃逸局部极小值。
...
#figure(
  ...
  pseudocode-list(booktabs: true, numbered-title: [Local Search])[
    + *while* elapsed time < cutoff *do*
      + $sigma arrow.l $ a random complete assignment
      + *if* $sigma "SAT" cal(F)$ *then*
        + *return* $sigma$
      + $C arrow.l $ an unsat clause chosen at random
...
WalkSATlm 特别适用于某些类型的 SAT 实例，如接近相变点的随机 3-SAT 问题，其中变量与子句的比例使得问题特别具有挑战性。与完备求解器相比，WalkSATlm 在处理大型实例时通常提供更快的解决方案，尽管它不能保证找到解。未来的研究可能探索自适应特征或与其他技术的结合，以提高局部搜索算法在 SAT 求解中的性能和适用性。
